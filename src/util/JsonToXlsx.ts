...
import {
  ...
  IJsonSheetRow,
  ISettings,
  IWorksheetColumnWidth,
} from "../types/JsonToXlsx";

export const getContentProperty = (
  content: IContent,
  property: string,
): string | number | boolean | Date | IContent => {
...
export const getWorksheetColumnWidths = (
  worksheet: WorkSheet,
  extraLength = 1,
): IWorksheetColumnWidth[] => {
  ...
  return columnLetters.map((column) => {
    ...
    return {
      width: (worksheet[maxWidthCell].v.length as number) + extraLength,
    };
  });
};

const getWorksheet = (
  jsonSheet: IJsonSheet,
  settings: ISettings,
): WorkSheet => {
...
const writeWorkbook = (
  ...
  const filetype = settings.writeOptions?.bookType
    ? settings.writeOptions.bookType
    : "xlsx";
  const filename = `${settings.fileName ?? "Spreadsheet"}.${filetype}`;
...
const JsonToXlsx = (
  jsonSheets: IJsonSheet[],
  settings: ISettings = {},
  workbookCallback: IWorkbookCallback = () => {},
): Buffer | undefined => {
