...
const ReportMonthly = () => {
  ...
  const [filter, setFilter] = useState({
    startDate: dayjs().startOf("month").format(DATE_FORMAT_API),
    endDate: dayjs().endOf("month").format(DATE_FORMAT_API),
    currentMonth: dayjs().format("MMMM"),
  });

  const handleDatePickerChange: DatePickerProps["onChange"] = (values: any) => {
    setFilter({
      startDate: dayjs(values).startOf("month").format(DATE_FORMAT_API),
      endDate: dayjs(values).endOf("month").format(DATE_FORMAT_API),
      currentMonth: dayjs(values).format("MMMM"),
    });
  };
  ...
  return (
    <>
      <PageHeader
        title={`Monthly Report of ${filter.currentMonth}`}
        right={
          <Form name="basic" form={form} autoComplete="off" layout="horizontal" style={{ display: "flex" }}>
            <Form.Item name="month" label="Month" style={{ marginRight: 8 }}>
              <DatePicker defaultValue={dayjs()} onChange={handleDatePickerChange} picker="month" allowClear={false} />
            </Form.Item>
          </Form>
        }
      />
